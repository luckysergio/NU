...
import { useProgramTarget } from "../../hooks/useProgramTarget";
import { X, Loader2, Target, Info } from "lucide-react";

const ProgramTargetModal = ({
  isOpen,
  onClose,
  editingTarget,
  onSuccess,
  ...
  const { create, update, isCreating, isUpdating } = useProgramTarget();
  ...
  useEffect(() => {
    if (editingTarget) {
      setFormData({
        nama: editingTarget.nama || "",
        is_active: editingTarget.is_active ?? true,
      });
    } else {
      setFormData({
        nama: "",
  ...
  const handleSubmit = async (e) => {
    ...
    const mutationOptions = {
      onSuccess: (result) => {
        success(
          "Berhasil",
          editingTarget
            ? "Sasaran program berhasil diperbarui"
            : "Sasaran program baru berhasil dibuat",
        );
        onClose();
    ...
    if (editingTarget) {
      update({ id: editingTarget.id, data: submitData }, mutationOptions);
    } else {
          ...
          <div className="relative flex justify-between items-center">
            <div>
              ...
              <p className="text-emerald-100 text-sm mt-0.5">
                {editingTarget
                  ? "Ubah data sasaran program"
                  : "Isi form berikut untuk menambahkan sasaran program baru"}
              </p>
            </div>
            <button
              onClick={onClose}
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-5">
            ...
            <div className="pt-3 border-t border-gray-100">
              ...
              <p className="mt-1 text-xs text-gray-500">
                Jika tidak aktif, sasaran ini tidak akan muncul di pilihan
                program kerja
              </p>
            </div>
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="text-xs text-blue-700">
                  ...
                  <ul className="space-y-1">
                    <li>
                      • Sasaran program digunakan untuk menentukan target dari
                      program kerja
                    </li>
                    <li>
                      • Hanya sasaran yang aktif yang bisa dipilih untuk program
                      kerja
                    </li>
                    <li>
                      • Sasaran yang sudah memiliki program kerja tidak dapat
                      dihapus
                    </li>
                  </ul>
                </div>
        ...
        <div className="sticky bottom-0 bg-white border-t border-gray-100 px-6 py-4 flex justify-end gap-3 rounded-b-2xl">
          ...
          >
            {submitting || isCreating || isUpdating ? (
              ...
              <span>{editingTarget ? "Update" : "Simpan"}</span>
            )}
          </button>
        </div>
      </div>
...
export default ProgramTargetModal;