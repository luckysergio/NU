...
const Anggotas = () => {
  ...
  const { data: organizationsData, isLoading: isLoadingOrgs } = useQuery({
    ...
    queryFn: async () => {
      const result = await organizationService.getAllSimple({ per_page: 1000 });
  ...
  const { organizations, filteredOrganizations, filteredJabatans } = useMemo(() => {
    ...
    if (filterLevel) {
      filteredOrgs = accessibleOrgs.filter((org) => getOrgLevelSlug(org) === filterLevel);
      filteredJabs = allJabatans.filter(
        (jab) => jab.level === filterLevel || (jab.levels && Array.isArray(jab.levels) && jab.levels.includes(filterLevel))
      );
    }
  ...
  }, [organizationsData, jabatansData, filterLevel, isSuperAdmin, userOrganizationId, isPCLevel, isMWCLevel, isRantingLevel]);
  ...
  const filters = useMemo(
    ...
    [page, perPage, debouncedSearch, filterLevel, filterOrganization, filterJabatan, filterStatus, userId]
  );
  ...
  const handleDelete = (anggota) => {
    ...
    // ✅ Ambil nama dengan aman dari biodata atau fallback ke accessor
    const namaAnggota = anggota.biodata?.nama || anggota.nama || "Anggota ini";

    warning(
      "Konfirmasi Hapus",
      `Apakah Anda yakin ingin menghapus keanggotaan "${namaAnggota}" dari organisasi ini?`,
      async () => {
        setActionLoading((prev) => ({ ...prev, [anggota.id]: true }));
        try {
          await deleteAnggota(anggota.id);
          success("Berhasil", "Keanggotaan berhasil dihapus");
  ...
  const hasActiveFilters = filterLevel || filterOrganization || filterJabatan || filterStatus || debouncedSearch;
      ...
      <MainLayout>
        <div className="min-h-screen bg-linear-to-br from-gray-50 to-gray-100 flex items-center justify-center">
          <div className="text-center">
            ...
            <button onClick={() => refetch()} className="mt-4 px-4 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 transition-colors">
              Coba Lagi
            </button>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">Kelola data anggota organisasi Nahdlatul Ulama</p>
            </div>
            {canCreate && (
              <button onClick={openCreateForm} className="inline-flex items-center gap-2 px-5 py-2.5 bg-linear-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 text-white rounded-xl transition-all duration-200 shadow-md hover:shadow-lg transform hover:scale-105">
          ...
          <div className="bg-white rounded-2xl shadow-lg overflow-hidden border border-gray-100 p-5 sm:p-6">
            <div className="mb-4">
              <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">CARI ANGGOTA</label>
              <div className="relative">
                ...
                />
                {searchQuery && (
                  <button type="button" onClick={handleClearSearch} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 transition-colors" aria-label="Clear search">
                    <X className="w-5 h-5" />
                  </button>
                )}
              </div>
            ...
            {showFilters && (
              <>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                  <div>
                    <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">LEVEL ORGANISASI</label>
                    <select value={filterLevel} onChange={handleFilterLevelChange} className="w-full px-3 py-2.5 border-2 border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-all duration-200 bg-white">
                      <option value="">Semua Level</option>
                      {availableLevelOptions.map((level) => (
                        <option key={level.slug} value={level.slug}>{level.display}</option>
                      ))}
                    </select>
                  </div>

                  <div>
                    <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">ORGANISASI</label>
                    <select value={filterOrganization} onChange={handleFilterChange(setFilterOrganization)} className="w-full px-3 py-2.5 border-2 border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-all duration-200 bg-white" disabled={!filterLevel}>
                      <option value="">Semua Organisasi</option>
                      {filteredOrganizations.map((org) => {
                        const orgLevel = typeof org.level === "string" ? org.level : org.level?.slug;
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">JABATAN</label>
                    <select value={filterJabatan} onChange={handleFilterChange(setFilterJabatan)} className="w-full px-3 py-2.5 border-2 border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-all duration-200 bg-white">
                  ...
                  <div>
                    ...
                    <select value={filterStatus} onChange={handleFilterChange(setFilterStatus)} className="w-full px-3 py-2.5 border-2 border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-all duration-200 bg-white">
                ...
                {hasActiveFilters && (
                  <div className="mt-4 flex justify-end">
                    <button onClick={handleReset} className="inline-flex items-center gap-2 px-4 py-2.5 border-2 border-gray-200 text-gray-700 rounded-xl hover:bg-gray-50 hover:border-gray-300 transition-all duration-200">
                      <X className="w-4 h-4" />
                      Reset Filter
                    </button>
                  </div>
                ...
                <table className="w-full">
                  <thead className="bg-linear-to-r from-gray-50 to-gray-100 border-b-2 border-gray-200">
                    <tr>
                      ...
                      {canManage && <th className="text-center px-6 py-4 text-xs font-semibold text-gray-600 uppercase tracking-wider">Aksi</th>}
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-100">
                    {anggotaList.length === 0 ? (
                      <tr>
                        <td colSpan={canManage ? 7 : 6} className="px-6 py-16 text-center">
                          <div className="flex flex-col items-center gap-2">
                            ...
                            {hasActiveFilters && (
                              <button onClick={handleReset} className="mt-2 text-emerald-600 hover:text-emerald-700 font-medium text-sm">
                                Reset Filter
                              </button>
                            )}
                      ...
                      anggotaList.map((anggota, index) => {
                        // ✅ Fallback aman: gunakan biodata jika ada, jika tidak gunakan accessor di root anggota
                        const biodata = anggota.biodata || anggota;
                        const nama = biodata.nama || "Tanpa Nama";
                        const noAnggota = biodata.no_anggota || "-";
                        const isActive = biodata.is_active ?? anggota.is_active ?? true;

                        return (
                          <tr key={anggota.id} className="hover:bg-gray-50 transition-colors duration-200">
                            <td className="text-center px-6 py-4 text-sm text-gray-600">{(page - 1) * perPage + index + 1}</td>
                            <td className="text-center px-6 py-4">
                              <span className="text-sm font-medium text-gray-800">{noAnggota}</span>
                            </td>
                            <td className="text-center px-6 py-4">
                            ...
                            <td className="text-center px-6 py-4">{getStatusBadge(isActive)}</td>
                            {canManage && (
                              <td className="text-center px-6 py-4">
                                <div className="flex items-center justify-center gap-2">
                                  <button onClick={() => openDetail(anggota)} className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-all duration-200" title="Detail">
                                    <Eye className="w-4 h-4" />
                                  </button>
                                  <button onClick={() => openEditForm(anggota)} className="p-2 text-emerald-600 hover:bg-emerald-50 rounded-lg transition-all duration-200" title="Edit">
                                    <Edit className="w-4 h-4" />
                                  </button>
                                  <button onClick={() => handleDelete(anggota)} className="p-2 text-red-600 hover:bg-red-50 rounded-lg transition-all duration-200" title="Hapus" disabled={actionLoading[anggota.id] || isDeleting}>
                                    {actionLoading[anggota.id] ? <Loader2 className="w-4 h-4 animate-spin" /> : <Trash2 className="w-4 h-4" />}
                                  </button>
                                </div>
                              </td>
                            )}
              ...
              {pagination.last_page > 1 && !isFetching && anggotaList.length > 0 && (
                <div className="flex flex-col sm:flex-row items-center justify-between gap-4 px-4 sm:px-6 py-4 border-t border-gray-100 bg-gray-50">
                  <div className="text-sm text-gray-500 order-2 sm:order-1">
                    Menampilkan {(page - 1) * perPage + 1} - {Math.min(page * perPage, pagination.total)} dari {pagination.total} data
                  </div>
                  <div className="flex gap-2 order-1 sm:order-2">
                    <button onClick={() => handlePageChange(page - 1)} disabled={page === 1} className="p-2 border border-gray-300 rounded-lg disabled:opacity-50 disabled:cursor-not-allowed hover:bg-white transition-all duration-200">
                      ...
                      {Array.from({ length: Math.min(5, pagination.last_page) }, (_, i) => {
                        ...
                        return (
                          <button
                            ...
                            className={`px-3 py-1 rounded-lg transition-all duration-200 ${page === pageNum ? "bg-linear-to-r from-emerald-600 to-teal-600 text-white shadow-md" : "border border-gray-300 hover:bg-white"}`}
                          >
                            {pageNum}
                          </button>
                        );
                    ...
                    <button onClick={() => handlePageChange(page + 1)} disabled={page === pagination.last_page} className="p-2 border border-gray-300 rounded-lg disabled:opacity-50 disabled:cursor-not-allowed hover:bg-white transition-all duration-200">
