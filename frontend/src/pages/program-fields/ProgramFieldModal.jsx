...
import { useProgramField } from "../../hooks/useProgramField";
import { X, Loader2, FolderTree, Info } from "lucide-react";

const ProgramFieldModal = ({
  isOpen,
  onClose,
  editingField,
  onSuccess,
  ...
  const { create, update, isCreating, isUpdating } = useProgramField();
  ...
  const handleSubmit = async (e) => {
    ...
    const mutationOptions = {
      onSuccess: (result) => {
        success(
          "Berhasil",
          editingField
            ? "Bidang program berhasil diperbarui"
            : "Bidang program baru berhasil dibuat",
        );
        onClose();
    ...
    if (editingField) {
      update({ id: editingField.id, data: submitData }, mutationOptions);
    } else {
          ...
          <div className="relative flex justify-between items-center">
            <div>
              <h2 className="text-xl font-bold text-white">
                {editingField ? "Edit Bidang Program" : "Tambah Bidang Program"}
              </h2>
              <p className="text-emerald-100 text-sm mt-0.5">
                {editingField
                  ? "Ubah data bidang program"
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-5">
            ...
            <div>
              ...
              <div className="relative">
                <FolderTree className="absolute left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 text-gray-400" />
                <input
                  type="text"
                  name="nama"
                  value={formData.nama}
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="text-xs text-blue-700">
                  ...
                  <ul className="space-y-1">
                    <li>
                      • Bidang program digunakan untuk mengelompokkan program
                      kerja
                    </li>
                    <li>
                      • Hanya bidang yang aktif yang bisa dipilih untuk program
                      kerja
                    </li>
                    <li>
                      • Bidang yang sudah memiliki program kerja tidak dapat
                      dihapus
                    </li>
                  </ul>
                </div>
        ...
        <div className="sticky bottom-0 bg-white border-t border-gray-100 px-6 py-4 flex justify-end gap-3 rounded-b-2xl">
          ...
          >
            {submitting || isCreating || isUpdating ? (
              ...
              <span>{editingField ? "Update" : "Simpan"}</span>
            )}
          </button>
        </div>
      </div>
...
export default ProgramFieldModal;