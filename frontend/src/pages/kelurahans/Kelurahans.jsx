...
import KelurahanModal from "./KelurahanModal";
...
const Kelurahans = () => {
  ...
  const { data: kotasData } = useQuery({
    queryKey: ["kotas-for-kelurahan-filter"],
    queryFn: async () => {
  ...
  const { data: kecamatansData } = useQuery({
    queryKey: ["kecamatans-for-kelurahan-filter"],
    queryFn: async () => {
  ...
  // ✅ Filter kecamatan berdasarkan kota yang dipilih (cascading)
  ...
  // ✅ Reset filter kecamatan saat kota berubah
  useEffect(() => {
    setFilterKecamatan("");
  }, [filterKota]);
  ...
  const filters = useMemo(
    ...
    [page, perPage, debouncedSearch, filterKecamatan],
  );
  ...
  const kelurahanList = response?.data || [];
  ...
  const handleDelete = (kelurahan) => {
    ...
    warning(
      ...
      async () => {
        setActionLoading((prev) => ({ ...prev, [kelurahan.id]: true }));
        try {
          await deleteKelurahan(kelurahan.id);
          success("Berhasil", "Kelurahan berhasil dihapus");

          if (kelurahanList.length === 1 && page > 1) {
            setPage(page - 1);
          }
        } catch (err) {
          ...
          error(
            ...
            err?.response?.data?.message ||
              err.message ||
              "Gagal menghapus kelurahan",
          );
        } finally {
          setActionLoading((prev) => ({ ...prev, [kelurahan.id]: false }));
        }
      },
    );
  };
  ...
  const hasActiveFilters = debouncedSearch || filterKota || filterKecamatan;
  ...
  return (
    <MainLayout>
                            ...
                            <td className="text-center px-6 py-4">
                              <div className="flex items-center justify-center gap-2">
                                ...
                                <button
                                  ...
                                  title="Hapus"
                                  disabled={
                                    actionLoading[kelurahan.id] || isDeleting
                                  }
                                >
                                  {actionLoading[kelurahan.id] ? (
      ...
      <KelurahanModal
        ...
        editingKelurahan={editingKelurahan}
