...
const Kecamatans = () => {
  ...
  const { data: kotasData } = useQuery({
    queryKey: ["kotas-for-kecamatan-filter"],
    queryFn: async () => {
  ...
  const filters = useMemo(
    ...
    [page, perPage, debouncedSearch, filterKota],
  );
  ...
  const handleDelete = (kecamatan) => {
    ...
    warning(
      ...
      async () => {
        setActionLoading((prev) => ({ ...prev, [kecamatan.id]: true }));
        try {
          await deleteKecamatan(kecamatan.id);
          success("Berhasil", "Kecamatan berhasil dihapus");

          if (kecamatanList.length === 1 && page > 1) {
            setPage(page - 1);
          }
        } catch (err) {
          ...
          setActionLoading((prev) => ({ ...prev, [kecamatan.id]: false }));
        }
      },
    );
  };
  ...
  const hasActiveFilters = debouncedSearch || filterKota;
  ...
  return (
    <MainLayout>
                            ...
                            <td className="text-center px-6 py-4">
                              <div className="flex items-center justify-center gap-2">
                                ...
                                <button
                                  ...
                                  title="Hapus"
                                  disabled={
                                    actionLoading[kecamatan.id] || isDeleting
                                  }
                                >
                                  {actionLoading[kecamatan.id] ? (
      ...
      <KecamatanModal
        ...
        editingKecamatan={editingKecamatan}
