...
const Attendance = () => {
  ...
  // =========================================================================
  // ✅ REACT QUERY - ATTENDANCE DETAIL (FIXED: Kurangi staleTime)
  // =========================================================================
  const {
    ...
    staleTime: 1000 * 30, // ✅ Kurangi dari 2 menit ke 30 detik
    gcTime: 1000 * 60 * 2,
    refetchOnWindowFocus: true, // ✅ Aktifkan refetch saat window focus
    enabled: isDetailView && !!id,
  });

  // =========================================================================
  // REACT QUERY - ALL ORGANIZATIONS
  // =========================================================================
  const {
  ...
  // =========================================================================
  // REACT QUERY - AVAILABLE ORGANIZATIONS
  // =========================================================================
  const {
  ...
  // Pilih data organisasi berdasarkan role
  ...
  // Filter organisasi berdasarkan search
  ...
  // =========================================================================
  // ✅ MUTATION - SAVE ATTENDANCE (FIXED: Proper Refetch)
  // =========================================================================
  const saveAttendanceMutation = useMutation({
    ...
    onError: (err, variables, context) => {
      ...
      if (context?.previousData) {
        queryClient.setQueryData(
          QUERY_KEYS.attendanceDetail(variables.activityId),
    ...
    onSettled: async (data, error, variables) => {
      ...
      // ✅ Manual refetch untuk memastikan data terbaru
      await refetchDetail();
  ...
  // =========================================================================
  // ✅ MUTATION - ADD PARTICIPANTS (FIXED: Proper Refetch)
  // =========================================================================
  const addParticipantsMutation = useMutation({
    ...
    onSettled: async (data, error, variables) => {
      ...
      if (!error) {
        ...
        error("Gagal", error?.response?.data?.message || error?.message || "Gagal menambahkan organisasi");
      }
    },
  });

  // =========================================================================
  // ✅ MUTATION - REMOVE PARTICIPANTS (FIXED: Proper Refetch)
  // =========================================================================
  const removeParticipantsMutation = useMutation({
    ...
    onSettled: async (data, error, variables) => {
      ...
      if (!error) {
        ...
        error("Gagal", error?.response?.data?.message || error?.message || "Gagal menghapus organisasi");
      }
    },
  });
  ...
  const handleToggleAttendance = (anggotaId) => {
    ...
    setAttendanceIds((prev) => {
      return prev.includes(anggotaId)
  ...
  const handleSelectAllOrganization = (orgId) => {
    ...
    setAttendanceIds((prev) => {
      if (allSelected) {
        return prev.filter((id) => !anggotaIds.includes(id));
      } else {
        const newIds = [...prev];
  ...
  const handleSaveAttendance = async () => {
    ...
    saveAttendanceMutation.mutate({
      activityId: attendanceDetail.activity.id,
      anggotaIds: attendanceIds,
    });
  };

  const handleToggleOrgSelect = (orgId) => {
    setSelectedOrgIds((prev) => {
      return prev.includes(orgId)
      ...
      <MainLayout>
        <div className="min-h-screen bg-linear-to-br from-gray-50 to-gray-100 flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-sm text-gray-500 mt-1">{errorActivities?.message || "Silakan coba lagi"}</p>
            <button
    ...
    return (
      <MainLayout>
        <div className="min-h-screen bg-linear-to-br from-gray-50 to-gray-100 flex items-center justify-center">
          <div className="text-center">
            <Loader2 className="w-12 h-12 text-emerald-600 animate-spin mx-auto mb-4" />
            <p className="text-gray-500">Memuat detail absensi...</p>
          </div>
        </div>
      </MainLayout>
    );
      ...
      <MainLayout>
        <div className="min-h-screen bg-linear-to-br from-gray-50 to-gray-100 flex items-center justify-center">
          <div className="text-center">
            <AlertCircle className="w-12 h-12 text-red-500 mx-auto mb-4" />
            <p className="text-gray-700">Terjadi kesalahan saat memuat data</p>
            <p className="text-sm text-gray-500 mt-1">{errorDetail?.message || "Silakan coba lagi"}</p>
            <button
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">
              Kelola absensi kegiatan organisasi Nahdlatul Ulama
            </p>
          </div>
        </div>
          ...
          <div>
            ...
            <div className="mt-2">
              ...
              <p className="text-sm text-gray-500">
                {formatDate(attendanceDetail.activity.tanggal_pelaksanaan)}
                {attendanceDetail.activity.tempat && ` • ${attendanceDetail.activity.tempat}`}
              </p>
            </div>
          </div>
            ...
            {canManage && (
              <>
                <button
                  ...
                  className="inline-flex items-center gap-2 px-4 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl transition-all duration-200 shadow-md hover:shadow-lg disabled:opacity-50"
                >
                ...
                <button
                  ...
                  disabled={saveAttendanceMutation.isPending || isRefreshing}
            ...
            {selectedOrganizations.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-gray-500 mb-3">
                  Belum ada organisasi peserta
                </p>
                {canManage && (
                  <button
                    onClick={handleOpenOrgModal}
                    disabled={isRefreshing}
                    className="inline-flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-all duration-200 disabled:opacity-50"
                  >
                    <Plus className="w-4 h-4" />
                    Tambah Organisasi
                  </button>
    ...
    return (
      <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full max-h-[85vh] overflow-hidden flex flex-col">
          <div className="relative bg-linear-to-r from-blue-600 to-blue-700 px-6 py-4 shrink-0">
            <div className="relative flex justify-between items-center">
              ...
              <button
                ...
                className="text-white/80 hover:text-white hover:bg-white/20 rounded-lg p-2 disabled:opacity-50"
              >
          ...
          <div className="p-4 border-b border-gray-100 bg-gray-50 shrink-0">
            <div className="relative mb-3">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2 border-2 border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm"
                disabled={addParticipantsMutation.isPending}
              />
            </div>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-gray-600">
                  ...
                  {orgSearch && filteredAvailableOrganizations.length !== availableOrganizations.length && (
            ...
            {isLoading ? (
              ...
              <div className="text-center py-8">
                ...
                <p className="text-gray-500">
                  {orgSearch ? "Tidak ada organisasi yang cocok dengan pencarian" : "Tidak ada organisasi yang tersedia"}
                </p>
              </div>
            ) : (
                ...
                {filteredAvailableOrganizations.map((org) => {
                  ...
                  return (
                    <button
                      ...
                      disabled={addParticipantsMutation.isPending}
                      className={`w-full flex items-center gap-3 p-3 border-2 rounded-xl transition-all duration-200 disabled:opacity-50 ${
                        isSelected
          ...
          <div className="p-4 border-t border-gray-100 bg-gray-50 shrink-0 flex gap-3">
            <button
              ...
              disabled={addParticipantsMutation.isPending}
              className="flex-1 px-4 py-2.5 border-2 border-gray-300 text-gray-700 rounded-xl hover:bg-gray-100 transition-all duration-200 disabled:opacity-50"
            >
              Batal
            </button>
            <button
              onClick={handleAddSelectedOrganizations}
              disabled={addParticipantsMutation.isPending || selectedOrgIds.length === 0}
