...
import {
  Calendar, Users, CheckCircle, XCircle, Clock, Search, ChevronLeft, ChevronRight,
  RefreshCw, Loader2, UserCheck, ArrowLeft, Save, Building2, Plus, AlertCircle, Info,
} from "lucide-react";
...
const Attendance = () => {
  ...
  // ✅ PERBAIKAN: Gunakan biodata_id untuk state absensi, bukan anggota_id
  const [attendedBiodataIds, setAttendedBiodataIds] = useState([]);
  ...
  const filters = useMemo(() => ({
    page, per_page: perPage, search: debouncedSearch || undefined, status: filterStatus || undefined,
  }), [page, perPage, debouncedSearch, filterStatus]);

  const { data: activitiesResponse, isLoading: isLoadingActivities, isFetching: isFetchingActivities, isError: isErrorActivities, error: errorActivities, refetch: refetchActivities } = useQuery({
    ...
    staleTime: 1000 * 60 * 5, gcTime: 1000 * 60 * 10, refetchOnWindowFocus: false, placeholderData: (previousData) => previousData, enabled: !isDetailView,
  });

  const activities = activitiesResponse?.data || [];
  const pagination = activitiesResponse || { current_page: 1, last_page: 1, per_page: 10, total: 0 };

  const { data: attendanceDetail, isLoading: isLoadingDetail, isFetching: isFetchingDetail, isError: isErrorDetail, error: errorDetail, refetch: refetchDetail } = useQuery({
    queryKey: QUERY_KEYS.attendanceDetail(id),
    queryFn: async () => {
      const result = await activityAttendanceService.getAttendance(parseInt(id));
      if (!result.success) throw new Error(result.message || "Gagal mengambil detail absensi");
      return result.data;
    },
    staleTime: 1000 * 30, gcTime: 1000 * 60 * 2, refetchOnWindowFocus: true, enabled: isDetailView && !!id,
  });

  const { data: allOrganizationsData, isLoading: isLoadingAllOrganizations, refetch: fetchAllOrganizations } = useQuery({
    ...
    staleTime: 1000 * 60 * 60 * 24, gcTime: 1000 * 60 * 60 * 24, refetchOnWindowFocus: false, enabled: isAdminRanting && isDetailView && showOrgModal,
  });

  const { data: availableOrgsData, isLoading: isLoadingAvailableOrgs, refetch: refetchAvailableOrgs } = useQuery({
    ...
    staleTime: 1000 * 60 * 5, gcTime: 1000 * 60 * 10, refetchOnWindowFocus: false, enabled: !isAdminRanting && isDetailView && !!id && showOrgModal,
  });
  ...
  const filteredAvailableOrganizations = useMemo(() => {
    ...
    return availableOrganizations.filter(org => org.nama?.toLowerCase().includes(searchLower) || org.level?.nama?.toLowerCase().includes(searchLower));
  }, [availableOrganizations, orgSearch]);

  // ✅ PERBAIKAN: Menggunakan biodataIds
  const saveAttendanceMutation = useMutation({
    ...
    onMutate: async (variables) => {
      ...
      queryClient.setQueryData(QUERY_KEYS.attendanceDetail(variables.activityId), (old) => {
        ...
        return {
          ...old,
          attended_biodata_ids: variables.biodataIds,
          attended_count: variables.biodataIds.length,
          attendance_percentage: old.total_participants > 0 ? Math.round((variables.biodataIds.length / old.total_participants) * 100) : 0,
        };
      });
      return { previousData };
    },
    onError: (err, variables, context) => {
      if (context?.previousData) queryClient.setQueryData(QUERY_KEYS.attendanceDetail(variables.activityId), context.previousData);
  ...
  // ✅ PERBAIKAN: Sinkronisasi state dari attended_biodata_ids
  useEffect(() => {
    if (attendanceDetail?.attended_biodata_ids) {
      setAttendedBiodataIds(attendanceDetail.attended_biodata_ids);
    }
  }, [attendanceDetail]);

  useEffect(() => {
    if (searchTimeoutRef.current) clearTimeout(searchTimeoutRef.current);
    searchTimeoutRef.current = setTimeout(() => { setDebouncedSearch(search); setPage(1); }, 500);
  ...
  // ✅ PERBAIKAN: Toggle berdasarkan biodata_id
  const handleToggleAttendance = (biodataId) => {
    if (!canManage) return;
    setAttendedBiodataIds((prev) => prev.includes(biodataId) ? prev.filter((id) => id !== biodataId) : [...prev, biodataId]);
  };

  // ✅ PERBAIKAN: Select all dalam satu organisasi berdasarkan biodata_id
  const handleSelectAllOrganization = (orgId) => {
    if (!canManage) return;
    const organization = selectedOrganizations.find((org) => org.id === orgId);
    if (!organization) return;
    const biodataIds = (organization.anggotas || []).map((a) => a.biodata_id);
    if (biodataIds.length === 0) return;
    
    const allSelected = biodataIds.every((id) => attendedBiodataIds.includes(id));
    setAttendedBiodataIds((prev) => {
      if (allSelected) return prev.filter((id) => !biodataIds.includes(id));
      const newIds = [...prev];
      biodataIds.forEach((id) => { if (!newIds.includes(id)) newIds.push(id); });
      return newIds;
    });
  };

  // ✅ PERBAIKAN: Select all global berdasarkan biodata_id
  const handleSelectAll = () => {
    if (!canManage) return;
    const allBiodataIds = [];
    selectedOrganizations.forEach((org) => {
      (org.anggotas || []).forEach((anggota) => allBiodataIds.push(anggota.biodata_id));
    });
    if (allBiodataIds.length === 0) return;
    const allSelected = allBiodataIds.every((id) => attendedBiodataIds.includes(id));
    setAttendedBiodataIds(allSelected ? [] : allBiodataIds);
  };

  // ✅ PERBAIKAN: Kirim biodata_ids ke mutation
  const handleSaveAttendance = () => {
    if (!attendanceDetail?.activity?.id || !canManage) {
      if (!canManage) warning("Info", "Anda tidak memiliki izin untuk menyimpan absensi");
      return;
    }
    saveAttendanceMutation.mutate({ activityId: attendanceDetail.activity.id, biodataIds: attendedBiodataIds });
  };
  ...
  const getStatusBadge = (status) => {
    const statusMap = { draft: { label: "Draft", color: "bg-gray-100 text-gray-600" }, completed: { label: "Selesai", color: "bg-emerald-100 text-emerald-700" }, cancelled: { label: "Dibatalkan", color: "bg-red-100 text-red-700" } };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold bg-linear-to-r from-emerald-600 to-teal-600 bg-clip-text text-transparent flex items-center gap-2">
            <Calendar className="w-8 h-8 text-emerald-600" /> Absensi Kegiatan
          </h1>
              ...
              <div className="flex gap-2 order-1 sm:order-2">
                ...
                <div className="flex gap-1">
                  {Array.from({ length: Math.min(5, pagination.last_page) }, (_, i) => {
                    ...
                    return <button key={pageNum} onClick={() => handlePageChange(pageNum)} className={`px-3 py-1 rounded-lg transition-all duration-200 ${page === pageNum ? "bg-linear-to-r from-emerald-600 to-teal-600 text-white shadow-md" : "border border-gray-300 hover:bg-white"}`}>{pageNum}</button>;
                  })}
                </div>
      ...
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <h1 className="text-2xl sm:text-3xl font-bold bg-linear-to-r from-emerald-600 to-teal-600 bg-clip-text text-transparent flex items-center gap-2">
              <UserCheck className="w-8 h-8 text-emerald-600" /> Absensi Kegiatan
            </h1>
            <div className="mt-2">
              ...
              <div className="divide-y divide-gray-100">
                {selectedOrganizations.length === 0 ? (
                  ...
                  selectedOrganizations.map((org) => {
                    ...
                    // ✅ PERBAIKAN: Hitung berdasarkan biodata_id
                    const orgBiodataIds = anggotaList.map((a) => a.biodata_id);
                    const orgAttendedCount = orgBiodataIds.filter((id) => attendedBiodataIds.includes(id)).length;
                    const isOrgAllSelected = orgBiodataIds.length > 0 && orgBiodataIds.every((id) => attendedBiodataIds.includes(id));
                      ...
                      <div key={org.id} className="p-4 hover:bg-gray-50 transition-colors">
                        <div className="flex items-center justify-between mb-3">
                          <div className="flex items-center gap-3">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-xs text-gray-400">({orgAttendedCount}/{orgBiodataIds.length})</span>
                            </div>
                          </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                          {anggotaList.map((anggota) => {
                            // ✅ PERBAIKAN: Gunakan biodata_id untuk key dan toggle
                            const isPresent = attendedBiodataIds.includes(anggota.biodata_id);
                            const nama = anggota.nama || anggota.biodata?.nama || "Tanpa Nama";
                            const jabatanNama = anggota.jabatan?.nama || "-";

                            return (
                              <button
                                key={anggota.biodata_id}
                                onClick={() => handleToggleAttendance(anggota.biodata_id)}
                                disabled={!canManage}
        ...
        <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full max-h-[85vh] overflow-hidden flex flex-col">
          <div className="relative bg-linear-to-r from-emerald-600 to-teal-600 px-6 py-4 shrink-0">
            <div className="relative flex justify-between items-center">
              <div>
                ...
                {isAdminRanting && (<p className="text-xs text-emerald-100 mt-1 flex items-center gap-1"><Info className="w-3 h-3" /> Admin Ranting: Bisa menambahkan organisasi manapun</p>)}
              </div>
          ...
          <div className="p-4 border-b border-gray-100 bg-gray-50 shrink-0">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-gray-600">{filteredAvailableOrganizations.length} organisasi tersedia{orgSearch && filteredAvailableOrganizations.length !== availableOrganizations.length && <span className="text-xs text-gray-400"> (filtered)</span>}</span>
              </div>
          ...
          <div className="p-4 overflow-y-auto flex-1">
            {isLoading ? (
              ...
              <div className="text-center py-8"><Building2 className="w-12 h-12 text-gray-300 mx-auto mb-3" /><p className="text-gray-500">{orgSearch ? "Tidak ada organisasi yang cocok dengan pencarian" : "Tidak ada organisasi yang tersedia"}</p></div>
            ) : (
              <div className="space-y-2">
                {filteredAvailableOrganizations.map((org) => {
                  const isSelected = selectedOrgIds.includes(org.id);
                  return (
                    <button key={org.id} onClick={() => handleToggleOrgSelect(org.id)} disabled={addParticipantsMutation.isPending} className={`w-full flex items-center gap-3 p-3 border-2 rounded-xl transition-all duration-200 disabled:opacity-50 ${isSelected ? "border-emerald-500 bg-emerald-50" : "border-gray-200 hover:border-emerald-300 hover:bg-gray-50"}`}>
                      ...
                      <div className="flex-1 text-left">
                        ...
                        {(org.level?.nama || org.level?.display_name) && (<span className="text-xs text-gray-400 ml-2">({org.level?.nama || org.level?.display_name})</span>)}
                      </div>
                    </button>
                  );
                })}
          ...
          <div className="p-4 border-t border-gray-100 bg-gray-50 shrink-0 flex gap-3">
            <button onClick={() => { setShowOrgModal(false); setSelectedOrgIds([]); setOrgSearch(""); }} disabled={addParticipantsMutation.isPending} className="flex-1 px-4 py-2.5 border-2 border-gray-300 text-gray-700 rounded-xl hover:bg-gray-100 transition-all duration-200 disabled:opacity-50">Batal</button>
            <button onClick={handleAddSelectedOrganizations} disabled={addParticipantsMutation.isPending || selectedOrgIds.length === 0} className="flex-1 px-4 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl transition-all duration-200 disabled:opacity-50 flex items-center justify-center gap-2">
              {addParticipantsMutation.isPending ? (<><Loader2 className="w-4 h-4 animate-spin" /> Menyimpan...</>) : (<><Plus className="w-4 h-4" /> Tambah {selectedOrgIds.length > 0 ? `(${selectedOrgIds.length})` : ""}</>)}
            </button>
          </div>
        </div>
      </div>
  ...
  if (isLoadingActivities && !isDetailView) return <MainLayout><div className="min-h-screen bg-linear-to-br from-gray-50 to-gray-100 flex items-center justify-center"><div className="text-center"><Loader2 className="w-12 h-12 text-emerald-600 animate-spin mx-auto mb-4" /><p className="text-gray-500">Memuat data...</p></div></div></MainLayout>;
  if (isErrorActivities && !isDetailView) return <MainLayout><div className="min-h-screen bg-linear-to-br from-gray-50 to-gray-100 flex items-center justify-center"><div className="text-center"><AlertCircle className="w-12 h-12 text-red-500 mx-auto mb-4" /><p className="text-gray-700">Terjadi kesalahan saat memuat data</p><p className="text-sm text-gray-500 mt-1">{errorActivities?.message || "Silakan coba lagi"}</p><button onClick={() => refetchActivities()} className="mt-4 px-4 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 transition-colors">Coba Lagi</button></div></div></MainLayout>;
  if (isLoadingDetail && isDetailView) return <MainLayout><div className="min-h-screen bg-linear-to-br from-gray-50 to-gray-100 flex items-center justify-center"><div className="text-center"><Loader2 className="w-12 h-12 text-emerald-600 animate-spin mx-auto mb-4" /><p className="text-gray-500">Memuat detail absensi...</p></div></div></MainLayout>;
  if (isErrorDetail && isDetailView) return <MainLayout><div className="min-h-screen bg-linear-to-br from-gray-50 to-gray-100 flex items-center justify-center"><div className="text-center"><AlertCircle className="w-12 h-12 text-red-500 mx-auto mb-4" /><p className="text-gray-700">Terjadi kesalahan saat memuat data</p><p className="text-sm text-gray-500 mt-1">{errorDetail?.message || "Silakan coba lagi"}</p><button onClick={() => refetchDetail()} className="mt-4 px-4 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 transition-colors">Coba Lagi</button></div></div></MainLayout>;
