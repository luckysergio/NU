...
const Jabatans = () => {
  ...
  const filters = useMemo(
    ...
    [page, perPage, debouncedSearch, filterLevel],
  );

  // ✅ Gunakan hook useJabatans dengan optimistic update
  const {
    data: response,
    isLoading,
    isFetching,
  ...
  const jabatanList = response?.data || [];
  ...
  const handleDelete = (jabatan) => {
    ...
    warning(
      ...
      async () => {
        setActionLoading((prev) => ({ ...prev, [jabatan.id]: true }));
        try {
          await deleteJabatan(jabatan.id);
          success("Berhasil", "Jabatan berhasil dihapus");

          if (jabatanList.length === 1 && page > 1) {
            setPage(page - 1);
          }
        } catch (err) {
          ...
          setActionLoading((prev) => ({ ...prev, [jabatan.id]: false }));
        }
      },
    );
  };
  ...
  const getLevelBadge = (levelSlug) => {
    ...
    const level = LEVEL_OPTIONS.find((l) => l.slug === levelSlug);
    ...
    return (
      <span
        className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${colors[levelSlug] || "bg-gray-100 text-gray-700"}`}
      >
        {level.display}
      </span>
    );
  ...
  const hasActiveFilters = debouncedSearch || filterLevel;
                            ...
                            <td className="text-center px-6 py-4">
                              <div className="flex items-center justify-center gap-2">
                                ...
                                <button
                                  ...
                                  title="Hapus"
                                  disabled={
                                    actionLoading[jabatan.id] || isDeleting
                                  }
                                >
                                  {actionLoading[jabatan.id] ? (
