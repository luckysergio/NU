...
const Roles = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (editingRole) {
      result = await roleService.update(editingRole.id, formData);
    } else {
      result = await roleService.create(formData);
    }
        ...
        <div className="max-w-7xl mx-auto space-y-6">
          ...
          <div className="relative">
            ...
            <div className={`bg-white rounded-2xl shadow-lg overflow-hidden border border-gray-100 transition-all duration-300 ${loading ? 'opacity-50' : 'opacity-100'}`}>
                        ...
                        <tr key={role.id} className="hover:bg-gray-50 transition-colors duration-200">
                          ...
                          <td className="text-center px-6 py-4">
                            <div className="flex flex-col items-center">
                              <span className={`inline-flex px-3 py-1.5 rounded-lg text-sm font-semibold ${getRoleBadgeColor(role.nama)}`}>
                                {role.nama}
                              </span>
              ...
              {/* Pagination */}
              {pagination.last_page > 1 && !loading && roles.length > 0 && (
