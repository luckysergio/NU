...
const Roles = () => {
  ...
  // ✅ Gunakan hook useRoles dengan optimistic update
  const {
    data: response,
    isLoading,
    isFetching,
  ...
  const handleDelete = (role) => {
    if (!isSuperAdmin) {
      error(
        "Akses Ditolak",
        "Hanya super admin yang dapat menghapus role",
      );
      return;
    }
  ...
  const openCreateForm = () => {
    if (!isSuperAdmin) {
      error(
        "Akses Ditolak",
        "Hanya super admin yang dapat menambah role",
      );
      return;
    }
    setEditingRole(null);
  ...
  const openEditForm = (role) => {
    if (!isSuperAdmin) {
      error(
        "Akses Ditolak",
        "Hanya super admin yang dapat mengedit role",
      );
      return;
    }
    setEditingRole(role);
                            ...
                            <td className="text-center px-6 py-4">
                              <div className="flex items-center justify-center gap-2">
                                ...
                                <button
                                  ...
                                  title="Hapus"
                                  disabled={
                                    actionLoading[role.id] || isDeleting
                                  }
                                >
