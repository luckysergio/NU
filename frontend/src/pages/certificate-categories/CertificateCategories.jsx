...
const CertificateCategories = () => {
  ...
  const handleDelete = (category) => {
    if (!canManage) {
      error(
        "Akses Ditolak",
        "Anda tidak memiliki izin untuk menghapus kategori sertifikat",
      );
      return;
    }
  ...
  const handleToggleStatus = async (category) => {
    ...
    setActionLoading((prev) => ({
      ...prev,
      [`toggle_${category.id}`]: true,
    }));
    try {
      await toggleStatus(category.id);
      success(
        "Berhasil",
        `Status berhasil diubah menjadi ${category.is_active ? "Tidak Aktif" : "Aktif"}`,
      );
    } catch (err) {
  ...
  const openCreateForm = () => {
    if (!canManage) {
      error(
        "Akses Ditolak",
        "Anda tidak memiliki izin untuk menambah kategori",
      );
      return;
    }
  ...
  const openEditForm = (category) => {
    if (!canManage) {
      error(
        "Akses Ditolak",
        "Anda tidak memiliki izin untuk mengedit kategori",
      );
      return;
    }
                        ...
                        >
                          <div className="flex flex-col items-center gap-2">
                            ...
                            <p className="text-gray-500">
                              Tidak ada data kategori sertifikat
                            </p>
                            {hasActiveFilters && (
                              <button
                                onClick={handleReset}
                            ...
                            <td className="text-center px-6 py-4">
                              <div className="flex items-center justify-center gap-2">
                                ...
                                <button
                                  ...
                                  title="Hapus"
                                  disabled={
                                    actionLoading[category.id] || isDeleting
                                  }
                                >
