// src/pages/organization-types/OrganizationTypes.jsx
...
const OrganizationTypes = () => {
  ...
  // Level options (hanya Lembaga dan Banom)
    ...
    const fetchLevels = async () => {
      ...
      try {
        ...
        if (result.success) {
          ...
          const filtered = result.data.data.filter(level => 
      ...
      async () => {
        try {
          ...
          if (result?.success === false) {
            error("Gagal", result?.message || "Gagal menghapus tipe organisasi");
            return;
          }
          success("Berhasil", result?.message || "Tipe organisasi berhasil dihapus");
        } catch (err) {
          console.error("Delete error:", err);
          error("Gagal", err?.response?.data?.message || err.message || "Gagal menghapus tipe organisasi");
        }
      }
    );
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingType) {
        result = await update({ id: editingType.id, data: submitData });
      } else {
      ...
      if (result?.data || result?.success === true) {
        const successMessage = editingType ? "Tipe organisasi berhasil diupdate" : "Tipe organisasi berhasil dibuat";
        ...
        <div className="max-w-7xl mx-auto space-y-6">
          ...
          <div className="relative">
            ...
            <div className={`bg-white rounded-2xl shadow-lg overflow-hidden border border-gray-100 transition-all duration-300 ${isFetching ? 'opacity-50' : 'opacity-100'}`}>
                        ...
                        <tr key={type.id} className="hover:bg-gray-50 transition-colors duration-200">
                          ...
                          <td className="text-center px-6 py-4">
                            <div>
                              ...
                              {type.level && type.level.nama !== type.nama && (
              ...
              {/* Pagination */}
              {pagination.last_page > 1 && !isFetching && types.length > 0 && (
              ...
              <div className="relative flex justify-between items-center">
                <div>
                  ...
                  <p className="text-emerald-100 text-sm mt-0.5">
                    {editingType ? "Ubah data tipe organisasi" : "Isi form berikut untuk menambahkan tipe organisasi baru"}
                  </p>
                </div>
                <button
                  onClick={closeForm}
